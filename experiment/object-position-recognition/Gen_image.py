...
class Gen_image:
    ...
    def circles(self, num_image):
        ...
        return self._base_geo(num_image, path="data/circles", geo_function=draw_circle)

    def squares(self, num_image):
        def draw_square(draw, x, y, r):
            draw.rectangle(((x-r, y-r), (x+r, y+r)),
                           fill=self._get_random_color())
        return self._base_geo(num_image, path="data/squares", geo_function=draw_square)
    ...
    def _base_geo(self, num_image, path, geo_function):
        ...
        for id in range(num_image):
            ...
            r = 4 #randint(ceil(self.size[0]/10), self.size[0]/2)
...
circle_locations = gen_image.circles(512)
square_locations = gen_image.squares(512)

save_csv(circle_locations, './data/circles/locations.csv')
